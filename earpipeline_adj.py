...
class EarPipeline:
    ...
    def run_whisper_inference(
        ...
        for audio, label in zip(tqdm(audio_paths_full), labels):
            try:
                if not label:
                    languages.append("")
    ...
    def inference(
        ...
        languages, segments_english, number_segments, speech_duration, segments_foreign = self.run_whisper_inference(audio_paths_full, labels)
